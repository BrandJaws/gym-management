...
trait FileUpload
{
    ...
    public function uploadShopCategoryImg(UploadedFile $uploadedFile, $modelObject, $propertyToUpdate, $filename = null, $objectId = null, $disk = "public")
    {
        $imageDelete = Image::where('image_type', 'App\ShopCategory')->where('image_id', $objectId)->get();
        if (count($imageDelete) > -1) {
            foreach ($imageDelete as $image) {
                $image_path = $image->path;
                if (file_exists($image_path)) {
    ...
    public function deleteShopCategoryImg($id, $disk = "public")
    {
        $imageDelete = Image::where('image_type', 'App\ShopCategory')->where('image_id', $id)->get();
        foreach ($imageDelete as $image) {
            $image_path = $image->path;
            if (file_exists($image_path)) {
                Storage::disk($disk)->delete($image_path);
    ...
    public function uploadGymLogo(UploadedFile $uploadedFile, $modelObject, $propertyToUpdate, $filename = null, $objectId = null, $disk = "public")
    {
        $imageDelete = Image::where('image_type', 'App\Gym')->where('image_id', $objectId)->get();
        if (count($imageDelete) > -1) {
            foreach ($imageDelete as $image) {
                $image_path = $image->path;
                if (file_exists($image_path)) {
            ...
            $folder = 'uploads/gym/logo' . strtolower(substr(get_class($modelObject), strrpos(get_class($modelObject), '\\') + 1)) . $psudoContainer;
            $file = $uploadedFile->storeAs($folder, sprintf('%s_%s', str_replace(' ', '_', $name), str_replace(' ', '_', microtime())) . '.' . $uploadedFile->getClientOriginalExtension(), [
                'disk' => $disk
            ]);
            $this->updateModelProperty($file, $modelObject, $propertyToUpdate, $disk);
    ...
    public function uploadSubCategory(UploadedFile $uploadedFile, $modelObject, $propertyToUpdate, $filename = null, $objectId = null, $disk = "public")
    {
        $imageDelete = Image::where('image_type', 'App\RestaurantSubCategory')->where('image_id', $objectId)->get();
        if (count($imageDelete) > -1) {
            foreach ($imageDelete as $image) {
                $image_path = $image->path;
                if (file_exists($image_path)) {
            ...
            $name = !is_null($filename) ? $filename : substr($uploadedFile->getClientOriginalName(), 0, strrpos($uploadedFile->getClientOriginalName(), "."));
            $psudoContainer = ($objectId) ? '/' . $objectId : '';
            $folder = 'uploads/gym/subCategory' . strtolower(substr(get_class($modelObject), strrpos(get_class($modelObject), '\\') + 1)) . $psudoContainer;
            $file = $uploadedFile->storeAs($folder, sprintf('%s_%s', str_replace(' ', '_', $name), str_replace(' ', '_', microtime())) . '.' . $uploadedFile->getClientOriginalExtension(), [
                'disk' => $disk
            ]);
            $this->updateModelProperty($file, $modelObject, $propertyToUpdate, $disk);
    ...
    public function deleteSubCategoryImg($id, $disk = "public")
    {
        $imageDelete = Image::where('image_type', 'App\RestaurantSubCategory')->where('image_id', $id)->get();
        foreach ($imageDelete as $image) {
            $image_path = $image->path;
            if (file_exists($image_path)) {
                Storage::disk($disk)->delete($image_path);
    ...
    public function uploadProduct(UploadedFile $uploadedFile, $modelObject, $propertyToUpdate, $filename = null, $objectId = null, $disk = "public")
    {
        $imageDelete = Image::where('image_type', 'App\RestaurantProduct')->where('image_id', $objectId)->get();
        if (count($imageDelete) > -1) {
            foreach ($imageDelete as $image) {
                $image_path = $image->path;
                if (file_exists($image_path)) {
    ...
    public function deleteProductImage($id, $disk = "public")
    {
        $imageDelete = Image::where('image_type', 'App\RestaurantProduct')->where('image_id', $id)->get();
        foreach ($imageDelete as $image) {
            $image_path = $image->path;
            if (file_exists($image_path)) {
                Storage::disk($disk)->delete($image_path);
